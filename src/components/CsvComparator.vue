            ...
            <template #cell="data">
                          <span :class="{
                              ...
                              <template v-if="showSourceValue && findDiffInRow(data.rowData.diff, data.column.id)">
                                <template
                                    v-if="findDiffInRow(data.rowData.diff, data.column.id).type === DiffType.MODIFIED">
                                    <el-text tag="del">{{
                                        findDiffInRow(data.rowData.diff, data.column.id).oldValue
                                      }}</el-text>
                                    -> {{ data.rowData[data.column.dataKey] }}
                                </template>
                                  <template
                                      v-else-if="findDiffInRow(data.rowData.diff, data.column.id).type === DiffType.REMOVED">
                                      {{ data.rowData[data.column.dataKey] }}
                                  </template>
                              </template>
                              <template v-else>
    ...
    <el-row v-if="csvDifferent"
      ...
      <el-statistic title="All changes" :value="statistics.allChanges">
        <template #suffix>
          ...
          <sup class="changes-stats_percentage">{{
              Math.floor(statistics.allChanges * 100 / statistics.totalCellsCount)
            }}%</sup>
        </template>
      </el-statistic>
